...
class Pwned:
    """
    All functions, with the exception of searchHashes(), return JSON
    formated data related to the function and arguments/search terms
    from https://haveibeenpwned.com. The searchHashes() fucntion will
    return a plaintext string of hashes.

    User must initialize the Pwned class with the account (email
    address being queried) and a User-Agent (the name of your
    application), even if an account is not applpicable to the
    function.
    ...
    IsSpamList    boolean   Indicates if the breach is considered a spam
       ...
       Usage::
         ...
         >>> data = foo.singleBreach(name='adobe')
    """
    ...
    def searchAllBreaches(self, truncate=False, domain=None, unverified=False):
        ...
        default, only verified breaches are returned web performing a
        search.
    ...
    def searchHashes(self, hsh):
        ...
        password. The downloadable source data delimits the full SHA-1
        hash and the password count with a colon (:) and each line with
        a CRLF.
